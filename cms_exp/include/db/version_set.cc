...
static void SaveValue(void* arg, const Slice& ikey, const Slice& v) {
  ...
  // std::cout<<"In save value: "<<parsed_key.user_key.ToString()<<" "<<s->ucmp->Compare(parsed_key.user_key, s->user_key)<<std::endl;
  if (!ParseInternalKey(ikey, &parsed_key)) {
    ...
    if (s->ucmp->Compare(parsed_key.user_key, s->user_key) == 0) {
      // std::cout<<"Hecked in if"<<std::endl;
...
std::string Version::DebugString() const {
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    ...
    r.append(" ---\n");
    std::cout<<"debugging string"<<std::endl;
    const std::vector<FileMetaData*>& files = files_[level];
    std::cout<<"Get file data"<<std::endl;
    for (size_t i = 0; i < files.size(); i++) {
